...
def generate_openapi_spec(path_prefix: str = ""):
    """
    Generate OpenAPI specification with Heimdall servers and path prefix.

    Args:
        path_prefix: API path prefix (e.g., "/heimdall-admin")
    """
    try:
        # Import FastAPI app
        ...
        # Add both production and staging servers
        openapi_schema["servers"] = [
            {
                "url": f"https://services.pref.rio{path_prefix}",
                "description": "Production server"
            },
            {
                "url": f"https://services.staging.app.dados.rio{path_prefix}",
                "description": "Staging server"
            }
        ]

        # Paths are kept as-is since FastAPI handles root_path automatically
        # The server URLs already include the path prefix
